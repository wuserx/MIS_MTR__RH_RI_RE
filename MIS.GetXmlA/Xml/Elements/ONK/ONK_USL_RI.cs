public class ONK_USL_RI
{
    private OnkUsl _onkusl;

    public ONK_USL_RI(OnkUsl onkusl)
    {
        _onkusl = onkusl;
    }
    ...
    public XElement Get()
    {
        IEnumerable<LEkPr> lekprs = new RepositoryMTR(new MtrContext()).GetAllLEkPrByOnkUslIdAsync(_onkusl.Id).Result;
        ...
        if(lekprs.Count() > 0)
        {
            ...
            {
                if (lekpr != null)
                    ONKUSL.Add(new LEK_PR_RI(lekpr).Get());
            }
        }
