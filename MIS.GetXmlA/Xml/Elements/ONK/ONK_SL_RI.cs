public class ONK_SL_RI
{
    private OnkSl_mtr _onksl;

    public ONK_SL_RI(OnkSl_mtr onksl)
    {
        _onksl = onksl;
    }
    ...
    public XElement Get()
    {
        IEnumerable<BDiag> bdiags = new RepositoryMTR(new MtrContext()).GetAllBDiagByOnkSlIdAsync(_onksl.Id).Result;
        IEnumerable<BProt> bprots = new RepositoryMTR(new MtrContext()).GetAllBProtByOnkSlIdAsync(_onksl.Id).Result;
        IEnumerable<OnkUsl> onkusls = new RepositoryMTR(new MtrContext()).GetAllOnkUslByOnkSlIdAsync(_onksl.Id).Result;
        ...
        {
            if (onkusl != null)
                ONKSL.Add(new ONK_USL_RI(onkusl).Get());
        }
