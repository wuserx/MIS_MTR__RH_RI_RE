...
public class Z_SL_RH
{
    public XElement Get(ZakSluch zakSluch)
    {
        ...
        else if (zakSluch.NPR_DATE != null)
            Z_SL.Add(new XElement("NPR_MO", zakSluch.LPU));

        if (zakSluch.NPR_DATE != null)
            Z_SL.Add(new XElement("NPR_DATE", zakSluch.NPR_DATE?.ToString("yyyy-MM-dd")));
        else
          if(zakSluch.FOR_POM == 3 && new List<int>() { 1, 2}.Contains( zakSluch.USL_OK ) )
            Z_SL.Add(new XElement("NPR_DATE", zakSluch.DATE_Z_1?.ToString("yyyy-MM-dd")));
        ...
        if (zakSluch.VBR != null)
            Z_SL.Add(new XElement("VBR", zakSluch.VBR));
        ...
        if (zakSluch.P_OTK != null)
            Z_SL.Add(new XElement("P_OTK", zakSluch.P_OTK));
        ...
        {            
            Z_SL.Add(new SL_RH_Xml(sluch).Get(zakSluch));            
        }
        ...
        if (zakSluch.SUMP != null)
            Z_SL.Add(new XElement("SUMP", RepeatedMEKs.Any() ? Math.Max(0, (decimal)(zakSluch?.SUMV - s_sum_RepeatedMEKs??0)) : Math.Max(0, (decimal)zakSluch.SUMP)) );
