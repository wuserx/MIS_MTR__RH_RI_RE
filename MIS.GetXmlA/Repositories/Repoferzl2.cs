...
public class Repoferzl2
{
    ...
    public Repoferzl2(ZakSluch zakSluch)
    {
    ...
    public Repoferzl2(ZakSluch_mtr zakSluch)
    {
        Surname = zakSluch.FAM;
        FirstName = zakSluch.IM;
        Patronymic = zakSluch.OT;
    ...
    public async Task<List<Policy>> GetPolicy()
    {
        try
        {
            ...
            Console.WriteLine(profiles.Any() ? "найден в enpn\n" : "не найден в enp");

            if (profiles.Count() > 0)
                return profiles;


            // Шаг 1: Поиск лиц по ФИО+региону
            var (policy, persons, pagination, requestId) = await FindPersonsByPersCriteriaFIODR();

            Console.WriteLine(policy.Any() ? "найден в find\n" : "не найден в find");

            if (policy == null || policy.Count() == 0)
            {
                Console.WriteLine("Пациент не найден в FindPersonsByPersCriteria.\n");
    ...
    private async Task<(List<Policy> policy, List<PersonShort> persons, PaginationInfo pagination, string requestId)> FindPersonsByPersCriteriaFIODR()
    {
        ...
        var response = await PostAsync(json, "UrlFindPersonsByPersCriteriaFiodr");
        if (!response.IsSuccessStatusCode)
            throw new HttpRequestException($"Ошибка поиска: {response.StatusCode}, {await response.Content.ReadAsStringAsync()}");
